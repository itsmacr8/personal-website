...
function searchMoviesMarkup(movie: DatabaseRecord, index: number) {
  return `
    <div class="movie-card" id=${index}>
        <div><img class="poster" src="${movie.Poster}" alt="${
    movie.Title
  } ${capitalize(movie.Type)} (${movie.Year}) Poster" title="${
    movie.Title
  } ${capitalize(movie.Type)} (${movie.Year}) Poster"></div>
        <div class="movie-card__body">
            <h2 class="movie-card__title">${
              movie.Title
            }<span class="title-text"> (${movie.Year})</span></h2>
            <p class="movie-text my-xs"><span class="point-name">Type:</span> ${capitalize(
              movie.Type
            )}</p>
            <div class="btn-group">
    ...
    movie.Title
        ...
        <div class="movie-card__body">
            ...
            <p class="my-xs"><span class="point-name">IMDB:</span> ${
              movie.IMDBRatings
            } | <span class="point-name">Rotten:</span> ${
    movie.RottenRatings
  }</p>
            ...
            <p><span class="point-name">Box Office:</span> ${
              movie.BoxOffice
            }</p>
            <a href="https://www.imdb.com/title/${
              movie.imdbID || movie.IMDB_ID
...
function detailsMovieMarkup(movie: MovieDetails) {
  return `
    ...
    movie.Title
  } ${capitalize(movie.Type)} (${movie.Year}) Poster">
